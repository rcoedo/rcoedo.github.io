...
date: 2019-01-18
footer_content: This article was [originally written for the Trabe medium publication](https://medium.com/trabe/node-js-grpc-services-in-style-222389be991a) , a colleciton of excellent articles written by [the awesome people from trabe.io](https://trabe.io/).
---
In the last years gRPC has been gaining a lot of traction. In this post we are going to cover the very basics of gRPC, and jump right into building a Greeter service with [grpc-node](https://github.com/grpc/grpc-node). After that, we’re going to rewrite it using [Mali](https://mali.js.org/).
...
## What is gRPC?
gRPC is a new take on the RPC paradigm to build APIs that recalls the old SOAP protocol. gRPC and SOAP are similar in that both use a contract which both client and service agree upon. However, gRPC handles quite well the known [SOAP weaknesses](https://en.wikipedia.org/wiki/SOAP#Technical_critique) while maintaining its strengths.
...
Using static generation we can pre-compile the proto definition into EcmaScript code (or any other supported language, actually) using [the protoc command line tool](https://www.systutorials.com/docs/linux/man/1-protoc/). Using static generation we get some generated data structures with input validation for the messages.
...
```
> grpcurl -plaintext -proto greeter.proto -d '{ "name": "Jude" }' 0.0.0.0:50051 GreeterService/GreetMe
{
  "reply": "Hello Jude!"
}
```
...
Just like we did before, we can use `grpcurl` to test it.
...
We’ve only covered unary operations, and we haven’t even mentioned how to generate the client’s code using the proto file, but that is left for future posts.
